...
if(marginal_dist=="negbin"){       
                    # Estimate the beta parameters of xT beta
                    fit <- glm.nb(y~., data=sim_data, link=log)
...
for(j in 1:num_iterations){
                    ...
                    if(optim_method=="nmkb"){    
                                        ...
                                        para <- nmkb(fn = Q_function, par = current, para.star=para.star, m=m, low=low, up=up, p=p, q=q, lower =lower_bound, upper = upper_bound)$par
                                        }
                                        if(pq==1){
